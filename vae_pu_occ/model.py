...
class myPU:
    ...
    def train_step_pn(
        ...
        if not case_control:
            pl_loss, pu1_loss, pu2_loss, u_loss = self.get_loss_components(
                pi_p,
                pi_prime,
                pi_pl,
                pi_pu,
            ...
            pl_loss, pu1_loss, pu2_loss, u_loss = self.get_loss_components_cc(
                pi_p,
                pi_prime,
                pi_pl,
                pi_pu,
    ...
    def get_loss_components(
        ...
        if balanced_risk:
            # logistic loss
            pl_loss = -pi_pl * (pi_prime / pi_p) * torch.mean(F.logsigmoid(pn_x_pl))
            pu1_loss = -pi_pu * (pi_prime / pi_p) * torch.mean(F.logsigmoid(pn_x_pu))
            pu2_loss = (
                pi_pu
                * ((1 - pi_prime) / (1 - pi_p))
                * torch.mean(F.logsigmoid(-pn_x_pu))
            )
            u_loss = (
                -pi_u
                * ((1 - pi_prime) / (1 - pi_p))
                * torch.mean(F.logsigmoid(-pn_x_u))
            )
        elif balanced_logit:
            pl_loss = pi_pl * torch.mean(
                torch.log(1 + torch.exp(-pn_x_pl) * (1 - pi_pl) / pi_pl)
            )
            pu1_loss = pi_pu * torch.mean(
                torch.log(1 + torch.exp(-pn_x_pu) * (1 - pi_pl) / pi_pl)
            )
            pu2_loss = -pi_pu * torch.mean(
                torch.log(1 + torch.exp(pn_x_pu) * pi_pl / (1 - pi_pl))
            )
            u_loss = pi_u * torch.mean(
                torch.log(1 + torch.exp(pn_x_u) * pi_pl / (1 - pi_pl))
            )
        elif unbalanced_savage:
            a = pi_pl / (1 - pi_pl)
            pl_loss = pi_pl * torch.mean(1 / (1 + a * torch.exp(pn_x_pl)) ** 2)
            pu1_loss = pi_pu * torch.mean(1 / (1 + a * torch.exp(pn_x_pu)) ** 2)
            pu2_loss = -pi_pu * torch.mean(1 / (1 + 1 / a * torch.exp(-pn_x_pu)) ** 2)
            u_loss = pi_u * torch.mean(1 / (1 + 1 / a * torch.exp(-pn_x_u)) ** 2)
        elif balanced_savage:
            a = pi_pl / (1 - pi_pl)
            pl_loss = pi_pl * torch.mean(1 / (1 + a * torch.exp(pn_x_pl)) ** 2)
            pu1_loss = pi_pu * torch.mean(1 / (1 + a * torch.exp(pn_x_pu)) ** 2)
            pu2_loss = -pi_pu * torch.mean(1 / (1 + 1 / a * torch.exp(-pn_x_pu)) ** 2)
            ...
            # logistic loss
            pl_loss = -pi_pl * torch.mean(F.logsigmoid(pn_x_pl))
            pu1_loss = -pi_pu * torch.mean(F.logsigmoid(pn_x_pu))
            pu2_loss = pi_pu * torch.mean(F.logsigmoid(-pn_x_pu))
            u_loss = -pi_u * torch.mean(F.logsigmoid(-pn_x_u))
        return pl_loss, pu1_loss, pu2_loss, u_loss

    def get_loss_components_cc(
        self,
        pi_p,
        pi_prime,
        pi_pl,
        ...
        pn_x_all = torch.cat([pn_x_pl, pn_x_u])

        if balanced_risk:
            # logistic loss
            pl_loss = -pi_pl * (
                (pi_prime / pi_p) * torch.mean(F.logsigmoid(pn_x_pl))
                - ((1 - pi_prime) / (1 - pi_p)) * torch.mean(F.logsigmoid(-pn_x_pl))
            )
            pu1_loss = -pi_pu * ((pi_prime / pi_p) * torch.mean(F.logsigmoid(pn_x_pu)))
            pu2_loss = pi_pu * (
                ((1 - pi_prime) / (1 - pi_p)) * torch.mean(F.logsigmoid(-pn_x_pu))
            )
            u_loss = -((1 - pi_prime) / (1 - pi_p)) * torch.mean(
                F.logsigmoid(-pn_x_all)
            )
        else:
            # logistic loss
            pl_loss = -pi_pl * torch.mean(F.logsigmoid(pn_x_pl))
            pu1_loss = -pi_pu * torch.mean(F.logsigmoid(pn_x_pu))
            pu2_loss = pi_pu * torch.mean(F.logsigmoid(-pn_x_pu))
            u_loss = -torch.mean(F.logsigmoid(-pn_x_all))
        return pl_loss, pu1_loss, pu2_loss, u_loss
    ...
    def accuracy(self, dataset, use_vae=False, balanced_cutoff=False, pi_p=None):
        ...
        for x, y in dataset:
            if use_vae:
                output = self.vae_predict(x)
                y_proba = torch.sigmoid(self.vae_predict(x))
            else:
                output = self.model_pn.classify(x, sigmoid=False)
                y_proba = self.model_pn.classify(x, sigmoid=True)

            ys.append(y)
            outputs.append(output)
            y_probas.append(y_proba)

        y = torch.concat(ys).detach().cpu().numpy()
        output = torch.concat(outputs).detach().cpu().numpy()
        y_proba = torch.concat(y_probas).detach().cpu().numpy()

        if not balanced_cutoff:
            y_pred = np.where(output > 0, 1, -1)
        else:
            y_pred = np.where(np.exp(output) > pi_p / (1 - pi_p), 1, -1)
        ...
        return acc, precision, recall, f1, auc, balanced_acc
